...
class User extends DB{
    ...
    public static function login($mailOrNickName,$password,$remember=0){
        $result=self::table("users")
                    ->whereRaw("userNickName=? OR userEmail=?",[$mailOrNickName,$mailOrNickName])
    ...
    public static function forgotPassword($mail,$siteUrl){
        ...
        if($result){
            ...
            if($passwordTokenAdd){
                ...
                <p><a href="'.$siteUrl.'forget-password/'.$token.'" style="background:#607d8b; color:#fff; width:120px; padding:8px; font-size:16px;">PAROLAMI YENİLE</a></p>
                <p>Güvenliğiniz için bu bağlantıyı paylaşmayınız</p>';
                return self::mail($result->userEmail,$result->userNameSurname,"Şifremi Unuttum",$message);
            }
            else{
                return false;
            }
    ...
    public static function resetPassword($mail,$password,$confirm_password){
        ...
        if($result){
            ...
            if($passwordUpdate){
                $tokensDelete=self::table("password_tokens")->where("userID",$result->ID)->delete();
